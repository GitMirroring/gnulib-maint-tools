...
static const char *
get_effective_type (glwthread_timedrwlock_t *lock)
{
  /* Lock for reading once.  */
  ASSERT (glwthread_timedrwlock_rdlock (lock) == 0);
  /* Lock for reading a second time.  */
  ASSERT (glwthread_timedrwlock_rdlock (lock) == 0);
  /* Unlock.  */
  ASSERT (glwthread_timedrwlock_unlock (lock) == 0);
  ASSERT (glwthread_timedrwlock_unlock (lock) == 0);

  /* Lock for writing once.  */
  ASSERT (glwthread_timedrwlock_wrlock (lock) == 0);

  /* Try to lock for writing a second time.  */
  int err = glwthread_timedrwlock_trywrlock (lock);
  if (err == 0)
    return "RECURSIVE";
  if (err == EBUSY)
    return "NORMAL";
